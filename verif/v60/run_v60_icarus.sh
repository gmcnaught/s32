...
declare -A TB=(
  ...
  [tb_v60_ce_alignment]="V60 CE ALIGNMENT PASS"
  [tb_v60_tstate]="V60 T-STATE PASS"
...
for tb in ${TBLIST:-$ORDER}; do
  ...
  # tb_v60_timebase deliberately drives an invalid divisor to prove the module
  # refuses it, so it needs the non-fatal build.
  extra_defs=""
  case "$tb" in tb_v60_timebase) extra_defs="-DV60_INVARIANT_NONFATAL" ;; esac
  if ! iverilog -g2012 -Wno-timescale -DSIMULATION $extra_defs -o "$WORK/$tb.vvp" -s "$tb" $PKG $CPU "$src" > "$WORK/$tb.log" 2>&1; then
